        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-3xl border-4 border-slate-950 p-6 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]">
                <form action="{{ route('admin.teachers.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Filter Peran (Role)</label>
                        <select name="role_id" onchange="this.form.submit()" class="w-full py-2.5 text-xs rounded-xl border-2 border-slate-950 focus:ring-2 focus:ring-slate-950 focus:border-slate-950 bg-white font-bold">
                    ...
                    <div class="flex items-end space-x-2">
                        ...
                        @if(request('search') || request('role_id'))
                            <a href="{{ route('admin.teachers.index') }}" class="text-xs font-black text-rose-600 hover:bg-[#FFEAEA] py-3 px-4 rounded-xl border-2 border-transparent hover:border-slate-950 transition-all uppercase tracking-wider">
                                Reset
                            </a>
                        @endif
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b-2 border-slate-950">
                                ...
                                <th class="px-6 py-4 text-xs font-black text-slate-950 uppercase tracking-wider">Peran (Role)</th>
                                <th class="px-6 py-4 text-xs font-black text-slate-950 uppercase tracking-wider">Kelas (Wali)</th>
                        ...
                        <tbody class="divide-y-2 divide-slate-950">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border-2 border-slate-950 shadow-[1px_1px_0px_0px_rgba(15,23,42,1)]
                                        @if($user->role->name === 'admin') bg-[#FFEAEA] text-rose-800
                                        @elseif($user->role->name === 'guru') bg-indigo-50 text-indigo-800
                                        @elseif($user->role->name === 'walikelas') bg-amber-50 text-amber-800
                                        @elseif($user->role->name === 'orangtua') bg-purple-50 text-purple-800
                                        @else bg-[#EAFCEF] text-emerald-800 @endif">
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="text-center py-12 text-slate-400 text-xs font-bold uppercase">Data user tidak ditemukan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div id="modal-add-user" class="uk-modal" uk-modal>
        <div class="uk-modal-dialog uk-modal-body border-4 border-slate-950 rounded-3xl shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] p-6 bg-white">
            ...
            <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Peran (Role)</label>
                    <select name="role_id" id="add-role-selector" onchange="toggleFormFields('add')" required class="w-full text-xs rounded-xl border-2 border-slate-950 focus:ring-2 focus:ring-slate-950 focus:border-slate-950 bg-white font-bold">
                        @foreach($roles as $role)
    ...
    <div id="modal-edit-user" class="uk-modal" uk-modal>
        <div class="uk-modal-dialog uk-modal-body border-4 border-slate-950 rounded-3xl shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] p-6 bg-white">
            ...
            <form id="edit-user-form" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Peran (Role)</label>
                    <select name="role_id" id="edit-role-selector" onchange="toggleFormFields('edit')" required class="w-full text-xs rounded-xl border-2 border-slate-950 focus:ring-2 focus:ring-slate-950 focus:border-slate-950 bg-white font-bold">
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" data-name="{{ $role->name }}">{{ $role->display_name }}</option>
                        @endforeach
                    </select>
