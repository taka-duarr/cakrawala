<x-app-layout>
            ...
            <div class="bg-white rounded-3xl border-4 border-slate-950 p-6 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]">
                <form action="{{ route('admin.parents.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-end space-x-2">
                        ...
                        @if(request('search'))
                            <a href="{{ route('admin.parents.index') }}" class="text-xs font-black text-rose-600 hover:bg-[#FFEAEA] py-3 px-4 rounded-xl border-2 border-transparent hover:border-slate-950 transition-all uppercase tracking-wider">
                                Reset
                            </a>
                        @endif
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b-2 border-slate-950">
                                ...
                                <th class="px-6 py-4 text-xs font-black text-slate-950 uppercase tracking-wider">Anak (Siswa)</th>
                        ...
                        <tbody class="divide-y-2 divide-slate-950">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-xs font-bold text-slate-700">
                                    ...
                                    @else
                                        <span class="text-slate-400 italic font-bold">Belum terhubung</span>
                                    @endif
                                </td>
    ...
    <div id="modal-add-user" class="uk-modal" uk-modal>
        <div class="uk-modal-dialog uk-modal-body border-4 border-slate-950 rounded-3xl shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] p-6 bg-white">
            ...
            <h2 class="text-lg font-black text-slate-950 mb-4 uppercase tracking-tight border-b-2 border-slate-950 pb-2">Tambah Orang Tua Baru</h2>
            <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4">
                ...
                <div id="add-child-field">
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Nama Anak (Siswa)</label>
                    <select name="parent_student_id" class="w-full text-xs rounded-xl border-2 border-slate-950 focus:ring-2 focus:ring-slate-950 focus:border-slate-950 bg-white font-bold">
        ...
        <div class="uk-modal-dialog uk-modal-body border-4 border-slate-950 rounded-3xl shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] p-6 bg-white">
            ...
            <form id="edit-user-form" method="POST" class="space-y-4">
                ...
                <div id="edit-child-field">
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Nama Anak (Siswa)</label>
                    <select name="parent_student_id" id="edit-child" class="w-full text-xs rounded-xl border-2 border-slate-950 focus:ring-2 focus:ring-slate-950 focus:border-slate-950 bg-white font-bold">
                        <option value="">-- Hubungkan Siswa --</option>
                        @foreach($students as $student)
                            <option value="{{ $student->id }}">{{ $student->name }} ({{ $student->classroom->name ?? 'Belum ada kelas' }})</option>
                        @endforeach
