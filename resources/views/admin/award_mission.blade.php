<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ route('admin.missions.index') }}" class="text-slate-500 hover:text-slate-700 transition">
                <span uk-icon="icon: arrow-left; ratio: 1.2"></span>
            </a>
            <div>
                <h2 class="font-bold text-2xl text-slate-800 leading-tight">Verifikasi Kelulusan Misi</h2>
                <p class="text-xs text-slate-400 mt-1 font-medium">Beri penghargaan poin kepada siswa yang menyelesaikan misi secara manual.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 p-6 soft-glow-indigo">
                <div class="flex items-start justify-between border-b border-slate-100 pb-4 mb-6">
                    <div>
                        <span class="px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-md text-[10px] font-bold uppercase tracking-wider">
                            Misi: {{ $mission->type }}
                        </span>
                        <h3 class="text-lg font-black text-slate-800 mt-2">{{ $mission->title }}</h3>
                        <p class="text-xs text-slate-400 font-medium mt-1">{{ $mission->description }}</p>
                    </div>
                    <div class="text-right">
                        <span class="text-xs text-slate-400 font-bold uppercase tracking-wider block">Hadiah</span>
                        <span class="text-2xl font-black text-emerald-600">+{{ $mission->points_reward }} Pts</span>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('admin.missions.award.process', $mission->id) }}" class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <h4 class="text-sm font-bold text-slate-700">Pilih Siswa yang Menyelesaikan Misi:</h4>
