<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ route('admin.missions.index') }}" class="inline-flex items-center justify-center w-10 h-10 border-2 border-slate-950 bg-white text-slate-950 hover:bg-[#E4FF1A] shadow-[2px_2px_0px_0px_rgba(15,23,42,1)] hover:-translate-y-0.5 active:translate-y-0.5 active:shadow-[1px_1px_0px_0px_rgba(15,23,42,1)] transition-all">
                <span uk-icon="icon: arrow-left; ratio: 1.2"></span>
            </a>
            <div>
                <h2 class="font-extrabold text-2xl text-slate-950 leading-tight tracking-tight">Verifikasi Kelulusan Misi</h2>
                <p class="text-xs text-slate-500 mt-1 font-bold">Beri penghargaan poin kepada siswa yang menyelesaikan misi secara manual.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white border-2 border-slate-950 p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)]">
                <div class="flex flex-col sm:flex-row sm:items-start justify-between border-b-2 border-slate-950 pb-4 mb-6 gap-4">
                    <div>
                        <span class="inline-block px-2.5 py-0.5 bg-[#FFEAEA] text-slate-950 border-2 border-slate-950 text-[10px] font-black uppercase tracking-wider shadow-[1px_1px_0px_0px_rgba(15,23,42,1)]">
                            Misi: {{ $mission->type }}
                        </span>
                        <h3 class="text-lg font-black text-slate-950 mt-2 tracking-tight">{{ $mission->title }}</h3>
                        <p class="text-xs text-slate-500 font-bold mt-1">{{ $mission->description }}</p>
                    </div>
                ...
                <form method="POST" action="{{ route('admin.missions.award.process', $mission->id) }}" class="space-y-6">
                    @csrf
                    
                    <div class="space-y-4">
                        <h4 class="text-sm font-extrabold text-slate-950 uppercase tracking-wider">Pilih Siswa yang Menyelesaikan Misi:</h4>
